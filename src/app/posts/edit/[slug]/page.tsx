...
const EditPost: React.FC<EditPostProps> = ({ params }) => {
		...
		const fetchPost = async () => {
            ...
            _updatedAt
				...
				if (fetchedPost) {
					...
					setCurrentStatus(fetchedPost.status || "draft"); // Set the current status
	...
	const handleSubmit = async (action: string) => {
		...
		if (currentStatus === "draft" && action === "published") {
			newStatus = "pending"; // Change status to pending if currently draft
		}
