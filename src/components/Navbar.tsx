...
const Navbar: React.FC = () => {
	...
	const clerk = useClerk(); // Initialize clerk
								...
								<div className="relative">
									...
									{isCogOpen && (
										<div className="absolute right-0 mt-2 w-40 bg-white dark:bg-gray-700 rounded-md shadow-lg py-2">
											<button
												onClick={() => clerk.signOut()} // Update signOut
												className="w-full text-left px-4 py-2 text-gray-800 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-600"
											>
												Logout
											</button>
											<button className="w-full text-left px-4 py-2 text-gray-800 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-600">
												Delete Account
											</button>
										</div>
									)}
